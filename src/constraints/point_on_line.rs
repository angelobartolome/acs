...
impl Constraint for PointOnLineConstraint {
    ...
    fn jacobian(&self, param_manager: &ParameterManager) -> DMatrix<f64> {
        ...
        let p1_x_idx = match p1_x_idx {
            Some(idx) => idx,
            None => return J,
        };
        let p1_y_idx = match p1_y_idx {
            Some(idx) => idx,
            None => return J,
        };
        let p2_x_idx = match p2_x_idx {
            Some(idx) => idx,
            None => return J,
        };
        let p2_y_idx = match p2_y_idx {
            Some(idx) => idx,
            None => return J,
        };
        let p3_x_idx = match p3_x_idx {
            Some(idx) => idx,
            None => return J,
        };
        let p3_y_idx = match p3_y_idx {
            Some(idx) => idx,
            None => return J,
        };
