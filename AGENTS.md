...
- `fully_constrained_ids` (JSON output) lists entities with 0 DOF, computed per connected component via `ConstraintSolver::fully_constrained_entity_ids()` (rank/null-space of the analytical Jacobian, reusing `ParametricDogLegSolver::build_system`). Points/circles/arcs are analyzed directly (fixed params count as locked); a line is included when both endpoints are. Only populated on a converged solve, `[]` otherwise. The demo tints these entities green (`renderer/theme.ts` `strokeForEntity`).
